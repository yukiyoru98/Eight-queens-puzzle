...
int const queens = 8;                // num of queens
...
{ // put queen in row x
    ...
    { // not all queens are placed yet

        for (int j = 0; j < queens; j++)
        {
            if ((column[j] == 0) && (diagonalLR[x - j + (queens - 1)] == 0) && (diagonalRL[x + j] == 0))
            { // this position is valid
                ...
                putQueen(x + 1); // place the queen for the next row
...
int main()
{
    ...
    putQueen(0); // create solution table
